 ...
 * @licence wxGameLib - v0.1.0 (2018-11-02)
...
(function (wxgame) {
    var Utils = /** @class */ (function () {
        ...
        Utils.catchScreenToTexSync = function (rect) {
            var sysInfo = wx.getSystemInfoSync();
            if (sysInfo["brand"] && (sysInfo["brand"].toUpperCase()).indexOf("HUAWEI") != -1) {
                return null;
            }
            if (rect) {
                var scaleX = uniLib.Global.screenWidth / sysInfo["screenWidth"];
                var scaleY = uniLib.Global.screenHeight / sysInfo["screenHeight"];
                return canvas.toTempFilePathSync({
                    x: rect.x * scaleX,
                    y: rect.y * scaleY,
                    width: rect.width * scaleX,
                    height: rect.height * scaleY,
                    destWidth: rect.width * sysInfo["pixelRatio"],
                    destHeight: rect.height * sysInfo["pixelRatio"],
                });
            }
            else {
                return canvas.toTempFilePathSync({});
