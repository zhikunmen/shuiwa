                ...
                for (var i = 0; i < this._rushData.length; i++) {
                    if (this._rushData[i].type == type) {
                        if (this._rushData[i].round) {
                            match.OnEnterRushHpMatchCmd_C(type, match.GameId.ID_COIN_FOUCA);
                        }
                        else {
                            uniLib.PopUpMgr.addPopUp(match.WxRushGameDetail, null, true, true, 0, uniLib.PopUpEffect.CENTER, 0, 0, { info: this._rushData[i], gameId: match.GameId.ID_COIN_FOUCA });
                        }
                        break;
                    }
                }
...
(function (match) {
    ...
    var MatchConst = (function () {
        ...
        MatchConst.RES_JSON = "resource/match/wxmatch.res_e66b8ea8.json";
        MatchConst.THM_JSON = "resource/match/gameEui_44d8401c.json";
        /**
         * 主界面需要加载的资源
         */
...
(function (match) {
    var WxBuyuReward = (function (_super) {
        ...
        WxBuyuReward.prototype.childrenCreated = function () {
            ...
            //    for(var i:number=0;i<fishType.fishType.length;i++){
            //         var fish:FishTypeItem = new FishTypeItem(fishType.fishType[i]);
            //         .addChild(fish);
            //         fish.x = 140*i;
            //    }
            for (var j = 0; j < 1; j++) {
                var item = new WxBuyuGoodItem(1);
                this.rewardContain.addChild(item);
                item.x = 110 * j;
            }
        };
        ...
        WxBuyuReward.prototype.enterGame = function (senceId, roomType) {
            LoadGameTipUtil.loadGame(this._data["gameId"], function (gameData) {
                MsgSendMgr.enterGame(gameData.gameId, senceId, roomType);
            });
        };
    ...
    var FishTypeItem = (function (_super) {
        ...
        FishTypeItem.prototype.dataChanged = function () {
            ...
            fish.texture = RES.getRes("fish_common_json.fish_" + this.data);
            this.addChild(fish);
        };
        return FishTypeItem;
    }(eui.ItemRenderer));
    ...
    var WxBuyuGoodItem = (function (_super) {
        ...
        function WxBuyuGoodItem(goodId) {
            var _this = _super.call(this) || this;
            _this._goodId = goodId;
            _this.initUI();
            return _this;
        }
        WxBuyuGoodItem.prototype.initUI = function () {
            ...
            gold.texture = RES.getRes("wx_lb_res_json.lb_jinbi_jinbi_png");
            ;
            this.addChild(gold);
            gold.x = (bg.width - gold.width) / 2;
            gold.y = (bg.height - gold.height) / 2;
            ...
            if (uniLib.UserInfo.goldChips >= data.lowestCarry) {
                ...
                var _loop_1 = function (i) {
                    if (uniLib.UserInfo.goldChips >= this_1._data.items[i].lowestCarry) {
                        var confirm_4 = new commonConfirm.ConfirmPanel("\u91D1\u5E01\u4E0D\u8DB3\uFF0C\u8BF7\u524D\u5F80" + obj[this_1._data.items[i].type] + "\u8FDB\u884C\u6E38\u620F!", null, null, function () {
