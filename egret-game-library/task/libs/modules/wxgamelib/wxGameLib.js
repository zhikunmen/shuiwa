...
(function (wxgame) {
    var Global = /** @class */ (function () {
        function Global() {
            ...
            this.onLoadBanner = function () { _this._bannerAd && _this._bannerAd.show(); };
            /**video unit id 数组*/
            this.videoUnitIdArr = ["adunit-ebbeeb9f7cbb945f"];
            this.onErrorVideo = function (err) {
                uniLib.TipsUtils.showTipsDownToUp("视频组件尚未准备好");
                                ...
                                fail: function () {
                                    wx.showModal({
                                        title: "授权提示", content: "获取授权失败，请确认授权！", showCancel: false, success: function () {
                                            wxgame.Global.instance.openSetting().then(function (authSetting) {
                                                if (authSetting[scopeStr]) {
                                                    resolve();
                                                }
                                                else {
        ...
        Global.prototype.onErrorBanner = function (err) {
            ...
            if (uniLib.Global.isWxGame()) {
                ...
                if (this._sysInfo) {
                    ...
                    this._bannerAd = wx["createBannerAd"]({
                        adUnitId: !!adUnitId ? adUnitId : this.bannerUnitIdArr[0], style: style ? style : {
        ...
        Global.prototype.createRewardedVideoAd = function (adUnitId, sucCall, callObj) {
            if (uniLib.Global.isWxGame()) {
                if (this._videoAd) {
                    ...
                    this._videoAd.show().then(function () { uniLib.SoundMgr.instance.pauseBgMusic(); }, function () { uniLib.TipsUtils.showTipsDownToUp("视频组件尚未准备好"); });
                }
                else {
    ...
    var Utils = /** @class */ (function () {
        ...
        Utils.getPosition = function (callBack, thisObj) {
            wxgame.Global.instance.authorize("scope.userLocation").then(function () {
                wx.getLocation({
                    success: function (res) {
                        var data = { code: 0, data: res };
                        callBack.call(thisObj, data);
                    },
                    fail: function (err) { uniLib.TipsUtils.showTipsDownToUp("定位失败，请重试" + err); }
                });
            }, function (err) {
        ...
        Utils.catchScreenToTexSync = function (width, height) {
            return canvas.toTempFilePathSync({ destWidth: width, destHeight: height });
        };
        return Utils;
    }());
    wxgame.Utils = Utils;
