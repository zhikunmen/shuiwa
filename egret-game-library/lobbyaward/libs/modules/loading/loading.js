...
(function (loading) {
    var LoadingConst = (function () {
        ...
        LoadingConst.RES_JSON = "resource/loading/loading.res_9df132bd.json";
        ...
        LoadingConst.RES_JSON_LAOYOU_GUANGDONG = "resource/loading_laoyou-guangdong.res.json";
        LoadingConst.RES_JSON_HCBUYU = "resource/loading_hcbuyu.res.json";
        LoadingConst.THM_JSON = "resource/loading/gameEui_182be519.json";
        /**
         * 公共loading需要加载的资源组
         */
...
(function (loading) {
     ...
     * 捕鱼加载不带版本号文字
     */
    var PublicLoadingHCBuYu = (function (_super) {
        __extends(PublicLoadingHCBuYu, _super);
        function PublicLoadingHCBuYu() {
            var _this = _super.call(this) || this;
        ...
        PublicLoadingHCBuYu.prototype.childrenCreated = function () {
            _super.prototype.childrenCreated.call(this);
            this.initUI();
        };
        PublicLoadingHCBuYu.prototype.initUI = function () {
        ...
        PublicLoadingHCBuYu.prototype.destroy = function () {
            if (this._timer) {
        ...
        PublicLoadingHCBuYu.prototype.setProgress = function (loaded, total, desc, resourceName, force) {
            var _this = this;
        ...
        return PublicLoadingHCBuYu;
    }(eui.Component));
    loading.PublicLoadingHCBuYu = PublicLoadingHCBuYu;
    __reflect(PublicLoadingHCBuYu.prototype, "loading.PublicLoadingHCBuYu");
})(loading || (loading = {}));
var loading;
(function (loading) {
    /**
     * 捕鱼加载带版本号文字
     */
    var PublicLoadingHCFish = (function (_super) {
        __extends(PublicLoadingHCFish, _super);
        function PublicLoadingHCFish() {
            var _this = _super.call(this) || this;
            _this.explain = "正在加载游戏资源...";
            _this._time = 0;
            _this.skinName = "PublicLoadingHCMahjongSkin";
        ...
        PublicLoadingHCFish.prototype.childrenCreated = function () {
            _super.prototype.childrenCreated.call(this);
            this.initUI();
        };
        PublicLoadingHCFish.prototype.initUI = function () {
            var logo = new egret.Bitmap();
            var text = new egret.Bitmap();
            if (uniLib.Global.isNative) {
                if (uniLib.Global.is_sandbox == 0) {
                    logo.texture = RES.getRes("xintiao_buyu_logo_png");
                    text.texture = RES.getRes("xintiao_load_txt_png");
                }
                else {
                    logo.texture = RES.getRes("jieji_buyu_logo_png");
                    text.texture = RES.getRes("jieji_load_txt_png");
                }
            }
            else {
                logo.texture = RES.getRes("haocai_buyu_logo_png");
                text.texture = RES.getRes("haocai_load_txt_png");
            }
            this.addChild(logo);
            logo.x = 70;
            logo.y = 30;
            this.addChild(text);
            text.x = (uniLib.Global.screenWidth - text.width) / 2;
            text.y = (uniLib.Global.screenHeight - text.height - 20);
        };
        PublicLoadingHCFish.prototype.destroy = function () {
            if (this._timer) {
                egret.clearInterval(this._timer);
            }
            uniLib.DisplayUtils.removeAllChildren(this);
        ...
        PublicLoadingHCFish.prototype.setProgress = function (loaded, total, desc, resourceName, force) {
            var _this = this;
            if (force === void 0) { force = false; }
            this._versionTxt.text = uniLib.Global.version + "";
            if (total && total != 0) {
        ...
        return PublicLoadingHCFish;
    }(eui.Component));
    loading.PublicLoadingHCFish = PublicLoadingHCFish;
    __reflect(PublicLoadingHCFish.prototype, "loading.PublicLoadingHCFish");
})(loading || (loading = {}));
var loading;
(function (loading) {
    /**
    ...
    var PublicLoadingLYView = (function (_super) {
        ...
        PublicLoadingLYView.prototype.setProgress = function (loaded, total, desc, resourceName, force) {
            ...
            if (total && total != 0) {
                ...
                var widthX = (this._darkLine_bmp.width - 14) * (num_4 / 100);
                this._lightLine_bmp.width = widthX;
            }
            else {
                this._explain_txf.text = desc;
...
(function (loading) {
    ...
    var PublicLoadingNBView = (function (_super) {
        ...
        PublicLoadingNBView.prototype.setProgress = function (loaded, total, desc, resourceName, force) {
            ...
            this._versionTxt.text = uniLib.Global.version + "";
            if (total && total != 0) {
                var num_5 = Math.ceil((loaded / total) * 100);
                if (force == false && num_5 > 93) {
                    num_5 = 93;
                }
                if (num_5 >= 93) {
                    if (!this._timer) {
                        this._timer = egret.setInterval(function () {
                            ...
                            if (_this._time >= 15 && uniLib.Global.isWxGame()) {
                                egret.clearInterval(_this._timer);
                                uniLib.Utils.clearLocalStorage();
                                wxgame.Utils.showConfirm(num_5 + ":网络异常,请重新登录!", "温馨提示", "确定", wxgame.Global.instance.exitMiniProgram);
                            }
                        }, this, 1000);
                    }
                }
                if (desc && desc != "") {
                    this._explain_txf.text = desc + (num_5 + "%");
                }
                else {
                    this._explain_txf.text = this.explain + (num_5 + "%");
                }
                var widthX = (this._darkLine_bmp.width - 14) * (num_5 / 100);
                this._lightLine_bmp.width = widthX;
            }
            else {
                this._explain_txf.text = desc;
...
(function (loading) {
    ...
    var PublicLoadingZZView = (function (_super) {
        ...
        PublicLoadingZZView.prototype.setProgress = function (loaded, total, desc, resourceName, force) {
            ...
            this._versionTxt.text = uniLib.Global.version + "";
            if (total && total != 0) {
                var num_6 = Math.ceil((loaded / total) * 100);
                if (force == false && num_6 > 93) {
                    num_6 = 93;
                }
                if (num_6 >= 93) {
                    if (!this._timer) {
                        this._timer = setInterval(function () {
                            ...
                            if (_this._time >= 15 && uniLib.Global.isWxGame()) {
                                ...
                                uniLib.Utils.clearLocalStorage();
                                wxgame.Utils.showConfirm(num_6 + ":网络异常,请重新登录!", "温馨提示", "确定", wxgame.Global.instance.exitMiniProgram);
                            }
                        }, 1000);
                    }
                }
                if (desc && desc != "") {
                    this._explain_txf.text = desc + (num_6 + "%");
                }
                else {
                    this._explain_txf.text = this.explain + (num_6 + "%");
                }
                var widthX = (this._darkLine_bmp.width) * (num_6 / 100);
                this._lightLine_bmp.width = widthX;
            }
            else {
                this._explain_txf.text = desc;
