...
(function (match) {
    ...
    var MatchConst = (function () {
        ...
        MatchConst.RES_JSON = "resource/match/wxmatch.res_5b4a8ec2.json";
        MatchConst.THM_JSON = "resource/match/gameEui_be984fb3.json";
        /**
         * 主界面需要加载的资源
         */
        ...
        WxGoldSelectType.prototype.onItemTapHandler = function (evt) {
            ...
            if (uniLib.UserInfo.goldChips >= data.lowestCarry) {
                if (!data.maxCarry || uniLib.UserInfo.goldChips <= data.maxCarry) {
                    ...
                    var confirm_2 = new commonConfirm.ConfirmPanel("金币超最大限制，快去挑战高级场次吧！", null, null, function () {
                    ...
                    uniLib.PopUpMgr.addPopUp(confirm_2, egret.MainContext.instance.stage, true, true, 0, uniLib.PopUpEffect.CENTER);
                }
            }
            else {
    ...
    var WxMahjongSelect = (function (_super) {
        ...
        WxMahjongSelect.prototype.enterGame = function (senceId) {
            LoadGameTipUtil.loadGame(this._gameId, function (gameData) {
                MsgSendMgr.enterGame(gameData.gameId, senceId);
            });
        };
        ...
        if (pigRush) {
            if (uniLib.PopUpMgr.hasPopup(match.WxPigRushThrough)) {
                ...
                LoadPanelTipMgr.getInstance().loadRes(match.MatchConst.HPW_PIG, function () {
                    uniLib.PopUpMgr.addPopUp(match.WxPigRushThrough, null, true, false, 0, uniLib.PopUpEffect.NOMAL, 0, 0, pigRush);
                });
            }
        }
        else if (rev.sInfos) {
...
(function (match) {
        ...
        if (scene) {
            if (type == 1 && scene.sInfos) {
                ...
                if (uniLib.PopUpMgr.hasPopup(match.WxPigRushThrough)) {
                    ...
                    LoadPanelTipMgr.getInstance().loadRes(match.MatchConst.HPW_PIG, function () {
                        uniLib.PopUpMgr.addPopUp(match.WxPigRushThrough, null, true, false, 0, uniLib.PopUpEffect.NOMAL, 0, 0, scene.cInfos);
                    });
                }
                return;
            }
                ...
                if (uniLib.PopUpMgr.hasPopup(match.WxPigRushThrough)) {
                    ...
                    LoadPanelTipMgr.getInstance().loadRes(match.MatchConst.HPW_PIG, function () {
                        uniLib.PopUpMgr.addPopUp(match.WxPigRushThrough, null, true, false, 0, uniLib.PopUpEffect.NOMAL, 0, 0, scene.rInfos);
                    });
                }
                return;
            }
   ...
   * 获取金币场选场数据
   */
    function OnEnterChallengeHpMatchCmd_C(type, round) {
...
(function (match) {
    ...
    var WxPigRushThrough = (function (_super) {
        ...
        function WxPigRushThrough(data) {
            var _this = _super.call(this) || this;
            if (data && data[0].items) {
                _this._pigData = data;
            }
            else {
                _this._rushData = data;
            }
            _this.width = uniLib.Global.screenWidth;
        ...
        WxPigRushThrough.prototype.onTouchHandler = function (evt) {
            if (evt.target == this.ret_btn) {
                ...
                for (var i = 0; i < this._pigData.length; i++) {
                    if (this._pigData[i].type == 1) {
                        uniLib.PopUpMgr.addPopUp(match.WxPigSelectPass, null, true, false, 0, uniLib.PopUpEffect.NOMAL, 0, 0, this._pigData[i]);
                        break;
                    }
                }
            }
            else if (evt.target == this.goldPig_btn) {
                for (var i = 0; i < this._pigData.length; i++) {
                    if (this._pigData[i].type == 2) {
                        uniLib.PopUpMgr.addPopUp(match.WxPigSelectPass, null, true, false, 0, uniLib.PopUpEffect.NOMAL, 0, 0, this._pigData[i]);
                        break;
                    }
                }
            ...
            else if (evt.target == this.cardRush_btn) {
                for (var i = 0; i < this._rushData.length; i++) {
                    if (this._rushData[i].type == 1) {
                        if (this._rushData[i].round) {
                            var req = new Cmd.EnterRushHpMatchCmd_C();
                            req.type = this._rushData[i].type;
                            NetMgr.tcpSend(req);
                        }
                        else {
                            uniLib.PopUpMgr.addPopUp(match.WxRushGameDetail, null, true, true, 0, uniLib.PopUpEffect.CENTER, 0, 0, this._rushData[i]);
                        }
                        break;
                    }
                }
            }
            else if (evt.target == this.goldRush_btn) {
                for (var i = 0; i < this._rushData.length; i++) {
                    if (this._rushData[i].type == 2) {
                        if (this._rushData[i].round) {
                            var req = new Cmd.EnterRushHpMatchCmd_C();
                            req.type = this._rushData[i].type;
                            NetMgr.tcpSend(req);
            ...
            else if (evt.target == this.diamondRush_btn) {
                for (var i = 0; i < this._rushData.length; i++) {
                    if (this._rushData[i].type == 3) {
                        if (this._rushData[i].round) {
                            var req = new Cmd.EnterRushHpMatchCmd_C();
                            req.type = this._rushData[i].type;
                            NetMgr.tcpSend(req);
        ...
        WxPigSelectPass.prototype.onTouchHandler = function (evt) {
            if (evt.target == this.ret_btn) {
                ...
                if (Array.isArray(config) && match.judgeItemIsEnough(config[0].goodId, config[0].goodNbr)) {
                    match.OnEnterChallengeHpMatchCmd_C(this._data.type, data.round);
        ...
        WxGameDetail.prototype.childrenCreated = function () {
            ...
            if (this._data) {
                ...
                if (config) {
                    ...
                    if (goodConfig) {
                        this.cost_lbl.text = "x" + config.HaoPaiMatchCost[0].goodNbr;
                    }
...
(function (match) {
    ...
    var WxRushGameDetail = (function (_super) {
        ...
        WxRushGameDetail.prototype.childrenCreated = function () {
            ...
            this.sign_btn.addEventListener(egret.TouchEvent.TOUCH_TAP, this.onTouchHandler, this);
            if (this._data) {
                this.rule_lbl.text = "1.\u7B2C" + this._data.reviveRounds.join("、") + "\u5173\u53EF\u590D\u6D3B\n2.\u5F69\u86CB\u6700\u9AD8\u53EF\u5F97" + this._data.maxGoldEgg + "\u798F\u5361";
                if (this._data.type == 1 || this._data.type == 3) {
                    this.title_lbl.text = "福卡闯关";
                    this.rewardType_img.source = "match_pig_json.cg_baoming_fuka";
                }
                else if (this._data.type == 2) {
                    this.title_lbl.text = "金币闯关";
                    this.rewardType_img.source = "match_pig_json.cg_baoming_jinbi";
                }
                else if (this._data.type == 3) {
                    this.title_lbl.text = "钻石闯关";
                    this.rewardType_img.source = "match_pig_json.cg_baoming_fuka";
                }
                var item = this._data.rewards[this._data.rewards.length - 1];
                this.lastRush_lbl.text = "\u7B2C" + this._data.rewards.length + "\u5173";
                this.lastReward_lbl.text = this.format(item);
                for (var i = 0; i < this._data.rewards.length - 1; i++) {
                    var lb = new eui.Label();
                    lb.fontFamily = "微软雅黑";
                    lb.size = 30;
                    lb.textFlow = [{ text: "\u7B2C" + (i + 1) + "\u5173:", style: { textColor: 0xff4e00 } }, { text: this.format(this._data.rewards[i]), style: { textColor: 0xa46359 } }];
                    this.rush_grp.addChild(lb);
                }
                var signFee = this._data.signFee[0];
                this.cost_lbl.text = "x" + (signFee.goodNbr > 1000 ? signFee.goodNbr / 10000 + "万" : signFee.goodNbr);
                if (signFee.goodId == match.GoodType.TYPE_DIAMOND) {
                    this.costType_img.source = "match_pig_json.cg_baoming_BMdiamond";
                }
                else if (signFee.goodId == match.GoodType.TYPE_FUKA) {
                    this.costType_img.source = "match_pig_json.cg_baoming_BMfuka";
                }
                else if (signFee.goodId == match.GoodType.TYPE_GOLD) {
                    this.costType_img.source = "match_pig_json.cg_baoming_BMgold";
                }
            }
        };
        WxRushGameDetail.prototype.format = function (items) {
            var egg = "";
            var str;
            for (var i = 0; i < items.rewards.length; i++) {
                str = str ? str + "+" : "";
                var item = items.rewards[i];
                if (item.goodId == match.GoodType.TYPE_DIAMOND) {
                    ...
                    str = (item.goodNbr > 10000 ? item.goodNbr / 10000 + "万" : item.goodNbr) + "\u91D1\u5E01";
                }
